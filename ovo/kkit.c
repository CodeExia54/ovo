...
static unsigned long (*ovo_kallsyms_lookup_name)(const char *symbol_name) = NULL;
...
static int resolve_kallsyms_lookup_name(void) {
    ...
    if (!ovo_kallsyms_lookup_name) {
        if (register_kprobe(&kp) < 0) {
            pr_err("[ovo] Failed to register kprobe for kallsyms_lookup_name\n");
            return -1;
        }
        ovo_kallsyms_lookup_name = (unsigned long (*)(const char *)) kp.addr;
        unregister_kprobe(&kp);
        pr_info("[ovo] kallsyms_lookup_name resolved at %px\n", ovo_kallsyms_lookup_name);
    }
    return 0;
#else
    ovo_kallsyms_lookup_name = kallsyms_lookup_name;
    pr_info("[ovo] kallsyms_lookup_name assigned direct kallsyms_lookup_name\n");
    return 0;
#endif
}
...
unsigned long *ovo_find_syscall_table(void) {
    ...
    table = (unsigned long *)ovo_kallsyms_lookup_name("sys_call_table");
